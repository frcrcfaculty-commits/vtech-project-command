...
export function SettingsPage() {
  ...
  // FIX: Removed updated_at from update payload — column does not exist in users table.
  // Also removed the brittle window.location.reload() hack; refreshUser() handles state update.
      ...
      <Card header={<span className="text-white font-semibold">Profile</span>}>
        <form onSubmit={handleSaveProfile} className="flex flex-col gap-4">
          ...
          <div className="flex items-center gap-2">
            <Input label="Role" value={user.role.replace(/_/g, ' ')} disabled className="flex-1" />
            <Badge status={user.role === 'owner' ? 'completed' : 'in_progress'} label={user.role.replace(/_/g, ' ')} className="mt-5" />
          </div>
