...
export function BOQPage() {
  ...
  const fetchBOQItems = async (projectId: string) => {
    ...
    try {
      ...
      console.error('Fetch BOQ error:', e);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      console.error('Save BOQ item error:', e);
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      console.error('Delete BOQ item error:', e);
    }
  };
    ...
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <Badge
            status={canSeePrices ? 'completed' : 'pending'}
            label={canSeePrices ? 'Prices visible' : 'Prices hidden'}
          />
          {canSeePrices
        ...
        <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-end">
          ...
          {selectedProject && (
            <div className="flex gap-3">
              <Badge status={selectedProject.status} label={selectedProject.status.replace('_', ' ')} />
            </div>
          )}
        </div>
      </Card>

      {/* Summary bar — prices only for privileged roles */}
      {selectedProjectId && (
            ...
            <>
              ...
              <Card padding="sm">
                <p className="text-2xl font-bold text-[var(--color-secondary)]">
                  {filteredItems.filter((i) => i.unit_price != null).length}
                </p>
          ...
          header={
            <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
              ...
              <div className="flex gap-2 w-full sm:w-auto">
                ...
                {canEdit && (
                  <Button icon={<Plus size={14} />} onClick={openAddModal} size="sm">
                    Add Item
                  </Button>
                )}
              </div>
                ...
                <tbody>
                  {filteredItems.map((item, idx) => (
                    ...
                    >
                      ...
                      <td className="py-3 px-2">
                        ...
                        {item.notes && (
                          <p className="text-xs text-[var(--color-text-secondary)] mt-0.5">{item.notes}</p>
                        )}
                      </td>
                      ...
                      <td className="py-3 px-2 text-right font-medium text-[var(--color-text)]">
                        ...
                        <td className="py-3 px-2">
                          <div className="flex gap-1 justify-end">
                            <button
                              ...
                              className="p-1.5 text-[var(--color-text-secondary)] hover:text-white hover:bg-white/8 rounded transition-colors"
                            >
                              <Edit2 size={13} />
                            </button>
                            <button
                              onClick={() => handleDelete(item.id)}
                              className="p-1.5 text-[var(--color-text-secondary)] hover:text-red-400 hover:bg-red-500/10 rounded transition-colors"
                            >
                              <Trash2 size={13} />
                            </button>
                          </div>
                  ...
                  {canSeePrices && filteredItems.length > 0 && (
                    <tr className="border-t border-white/15">
                      <td colSpan={canEdit ? 7 : 6} className="py-3 px-2 text-right font-semibold text-[var(--color-text)]">
                        Grand Total
                      </td>
                      <td className="py-3 px-2 text-right font-bold text-xl text-emerald-400">
                        ₹{totalValue.toLocaleString('en-IN', { maximumFractionDigits: 0 })}
                      </td>
                      {canEdit && <td />}
                    </tr>
      ...
      >
        <div className="flex flex-col gap-4">
          <Input
            ...
            placeholder="e.g. 85\" 4K Display Panel"
            required
          />
          ...
          {/* Unit price only shown to price-visible roles */}
          {canSeePrices && (
            <Input
              label="Unit Price (₹)"
              type="number"
              ...
              placeholder="Leave blank if not yet priced"
            />
          )}
          <Input
            label="Notes"
            ...
            placeholder="Specifications, installation notes..."
          />
