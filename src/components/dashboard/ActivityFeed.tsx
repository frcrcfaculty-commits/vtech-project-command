...
const typeConfig: Record<string, { icon: typeof Clock; color: string; bg: string }> = {
  time: { icon: Clock, color: 'text-blue-600', bg: 'bg-blue-500/15' },
  task: { icon: CheckCircle2, color: 'text-green-600', bg: 'bg-green-500/15' },
...
export function ActivityFeed() {
  ...
  if (!activities.length) return <div className="bg-white/5 rounded-lg shadow-sm border border-white/8 p-4 h-full"><EmptyState title="No activity" description="No recent activity found" /></div>;
      ...
      <ul className="flex-1 space-y-1 overflow-y-auto max-h-[400px]">
        {activities.map((item, i: number) => {
          ...
          return (
            ...
            >
              {/* Avatar */}
              <div className="flex-shrink-0 w-9 h-9 rounded-full bg-[var(--color-primary,#723B8F)] flex items-center justify-center text-white text-xs font-bold">
                {initials(item.user)}
              </div>
