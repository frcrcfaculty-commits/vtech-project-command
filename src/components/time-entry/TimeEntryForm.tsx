...
export function TimeEntryForm({ userId }: { userId: string }) {
  ...
  const loadProjects = async () => {
    ...
    if (data) setProjects(data as { id: string; name: string; client_name: string }[]);
  };
  ...
  const handleProjectChange = async (projId: string) => {
    ...
    if (data) {
      setPhases(data as { id: string; phase_name: string; status: string }[]);
  ...
  const handlePhaseChange = async (phaseId: string) => {
    ...
    if (data) setTasks(data as { id: string; title: string }[]);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (error) {
      setErrorMsg(typeof error === 'string' ? error : (error as { message?: string })?.message || 'An error occurred');
      setLoading(false);
    } else {
